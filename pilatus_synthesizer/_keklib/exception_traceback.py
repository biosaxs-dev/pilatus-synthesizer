"""Exception traceback formatting utility.

Original: KekLib/ExceptionTracebacker.py
...
class ExceptionTracebacker:
    def __init__(self, call_stack=False):
        (exc_type_, exc_value, exc_traceback) = sys.exc_info()
        self.c_seq = traceback.format_stack()[:-2] if call_stack else []
        self.e_seq = traceback.format_exception(exc_type_, exc_value, exc_traceback)[1:]
