"""New-style log file parsing and data array construction.

Handles the newer Pilatus naming convention (prefix_dN_NNNNN.ext).

Original: lib/PilatusUtilsNewStyle.py
Copyright (c) 2015-2025, SAXS Team, KEK-PF
"""

import os
import re
from .pilatus_utils_old_style import get_prefix_info_list, _get_dict_info
...
def get_data_info(folder_info, log_file, mask_file,
    ...
    counter_dict, org_file_dict, adj_file_dict, syn_file_dict = _get_dict_info(
        in_folder, adj_folder, syn_folder, pilatus_counter, counter_id
    )
    ...
    prefix_dict = {}
    for k, info in enumerate(prefix_info_list):
    ...
    for k, path in enumerate(sorted(image_files)):
        ...
        if single_image:
            ...
            key = '_'.join([f1, f3])
        num_changes_dict[key] = num_changes
    ...
    for edate, data_dict in sorted(exec_dict.items()):
        for key, value in sorted(data_dict.items(), key=alphanum_key):
            data_array.append([key, value, num_changes_dict[key]])
