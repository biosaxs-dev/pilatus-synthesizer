...
def tk_set_icon_portable( parent, name=None, module=None, use_cashed_image=False ):
    ...
    iconpath = os.path.dirname( os.path.abspath( mod.__file__ ) ) + '/%s' % ( name )
    try:
        if use_cashed_image:
            global cashed_image
            if cashed_image is None:
                iconpath += '.png'
                if not os.path.exists(iconpath): return
                print( 'Loading the icon image' )
                cashed_image = Tk.Image( "photo", file=iconpath )
            parent.tk.call( 'wm', 'iconphoto', parent._w, cashed_image )
        else:
            if os.name == 'nt':         # i.e. on Windows
                iconpath += '.ico'
                if not os.path.exists(iconpath): return
                parent.iconbitmap( iconpath )
            else:
                iconpath += '.png'
                if not os.path.exists(iconpath): return
        ...
        logging.getLogger().warning(str(etb))
