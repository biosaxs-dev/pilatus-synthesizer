...
class Controller( Tk.Toplevel ):
    def __init__( self, root, opts=None ):
        ...
        menu.add_command( label="Detector", command=self.detector_settings )
...
#        help.add_command( label="About",    command=self.about )
    ...
    def check_detector_settings(self):
        positive_direction = get_setting('positive_direction')
        print('check_detector_settings: positive_direction =', positive_direction )
        if positive_direction is None or positive_direction in ['', 'None']:
            MessageBox.showwarning("Detector Settings Required",
                                   "Later versions of Synthesizer require 'Positive Adjust Direction' setting for proper operation.\n\n"
                                   "Please set 'Positive Adjust Direction' in 'Settings' -> 'Detector' menu.")
            return False
        return True
    ...
    def detector_settings(self, debug=True):
        if debug:
            from importlib import reload
            import DetectorSettings
            reload(DetectorSettings)
        from DetectorSettings import DetectorSettings
        dialog = DetectorSettings(self.root, "Detector Settings")
        return  
